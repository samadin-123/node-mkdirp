...
Improve the performance of mkdirp directory creation operations (measured in ops/sec).

## What you CAN modify

- `src/*.ts` — all TypeScript source files including index.ts, mkdirp-native.ts, mkdirp-manual.ts, opts-arg.ts, path-arg.ts, use-native.ts, find-made.ts, and bin.ts
...
- `test/` — test suite
- `benchmark/` — benchmark harness (if present)
